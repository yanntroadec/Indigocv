    ...
    <div className="space-y-6">
      {experiences.length === 0 && (
        <p className="text-sm text-gray-500 text-center py-4">
          Aucune expérience ajoutée. Cliquez sur le bouton ci-dessous pour en ajouter une.
        </p>
      )}
      ...
      {experiences.map((exp, index) => (
        <div key={exp.id} className="border border-gray-200 rounded-xl p-4 space-y-3 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Poste" value={exp.role} onChange={(v) => update(exp.id, 'role', v)} placeholder="Développeur Web" />
            <Field label="Entreprise" value={exp.company} onChange={(v) => update(exp.id, 'company', v)} placeholder="Acme Corp" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Date de début" value={exp.startDate} onChange={(v) => update(exp.id, 'startDate', v)} placeholder="janv. 2022" />
            <div>
              <Field
                label="Date de fin"
                value={exp.endDate}
              ...
              <label className="flex items-center gap-2 mt-1 cursor-pointer">
                <input
                  ...
                  className="rounded"
                />
                <span className="text-xs text-gray-600">Poste actuel</span>
              </label>
            </div>
          </div>
