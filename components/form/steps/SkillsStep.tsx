...
export default function SkillsStep() {
  ...
  // Migrate old persisted format: string[] → { name, level }[], then filter invalid entries
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Ajouter une compétence</label>
      ...
      {skills.length > 0 && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-400">Cliquez sur les points pour indiquer le niveau (1–5)</p>
        </div>
      )}

      {skills.length === 0 && (
        <p className="text-sm text-gray-400 text-center py-4">Aucune compétence ajoutée pour l&apos;instant.</p>
      )}
    </div>
  )
}
