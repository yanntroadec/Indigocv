...
export default function PersonalStep() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label={t('email')} type="email" value={personal.email} onChange={(v) => update('email', v)} placeholder={t('emailPlaceholder')} />
      <Field label={t('phone')} type="tel" value={personal.phone} onChange={(v) => update('phone', v)} placeholder={t('phonePlaceholder')} />
      <Field label={t('street')} value={personal.street} onChange={(v) => update('street', v)} placeholder={t('streetPlaceholder')} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label={t('city')} value={personal.city} onChange={(v) => update('city', v)} placeholder={t('cityPlaceholder')} />
        <Field label={t('postalCode')} value={personal.postalCode} onChange={(v) => update('postalCode', v)} placeholder={t('postalCodePlaceholder')} />
      </div>
